...
router.get('/', (req, res) => {
  studentRepo.getStudents(result => {
    res.render('students/index', 
      { students: result.data, message: result.err });
  });
});
...
router.post('/new', (req, res) => {
  studentRepo.createStudent(req.body, result => {
    if (result.err) res.render('students/new', 
      { message: result.err });
...
router.get('/:id', (req, res) => {
  studentRepo.getStudent(req.params.id, (result) => {
    res.render('students/show', 
      { student: result.data, message: result.err });
  });
});
...
router.post('/:id', (req, res) => {
  studentRepo.updateTeacher(req.params.id, req.body, (result) => {
    if (result.err) 
      res.render('students/edit/' + req.params.id, 
        { message: result.err });
...
router.get('/edit/:id', (req, res) => {
  studentRepo.getStudent(req.params.id, (result) => {
    if (result.err) res.render('students/edit/' + req.params.id, 
